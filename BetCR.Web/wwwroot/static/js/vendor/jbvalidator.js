(function ($) {
    ...
    $.fn.jbvalidator = function (options) {
        let defaults = {
            ...
            successMessageHandler: function (el) {
        ...
        let errorMessages = {
            ...
            maxLength: "Please lengthen this text to %s characters or less.",
            minLength: "Please lengthen this text to %s characters or more.",
        ...
        $(FORM).on('serialize', function (event) {
            STATUS = 0;
        ...
        $(FORM).find(selector).each((i, el) => {
            $(el).on('input', function (event) {
                ...
                if (hasAttr(el, 'data-v-affect')) {
                    let effect = $(el).attr('data-v-affect');
                    let elements = effect.split(' ');
                    $.each(elements,
                        function (i, item) {
                            $(item).trigger('input');
                        });
        ...
        let validationRun = function (el, event) {
            ...
            if (el.checkValidity() === false) {
                options.errorMessageHandler(el, el.validationMessage);
                STATUS++;
            } else {
                options.successMessageHandler(el);
            }
        };
